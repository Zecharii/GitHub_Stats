...
def analytics():
    ...
    return render_template('analytics.html', username=username, name=user.name, avatar_url=user.avatar_url, bio=user.bio, 
                           ...
                           plot_url=plot_url, donut_chart_url=donut_chart_url, issues_heatmap_url=issues_heatmap_url, commits_heatmap_url=commits_heatmap_url, repos=repos)
...
# Generate pie chart for repo languages
...
def analytics():
    ...
    if not user:
        ...
        if response.status_code == 200:
            ...
            flash('An error occurred while fetching data from GitHub. Please try again later.', 'error')
            return redirect(url_for('index'))
    ...
    # Plotting
    plt.figure(figsize=(8, 4))
    plt.bar(['Followers', 'Public Repos'], [user.followers, user.public_repos])
    ...
    if repos:
        ...
        sns.heatmap(issues_data.set_index('Repo'), annot=True, fmt="d", cmap='YlGnBu')
        plt.title('Heatmap of Issues by Repo')
        img = io.BytesIO()
        plt.savefig(img, format='png')
        img.seek(0)
    ...
    return render_template('analytics.html', username=username, name=user.name, avatar_url=user.avatar_url, bio=user.bio, 
                           location=user.location, followers=user.followers, following=user.following, public_repos=user.public_repos,
                           plot_url=plot_url, pie_chart_url=pie_chart_url, heatmap_url=heatmap_url, repos=repos)
